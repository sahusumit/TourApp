...
exports.login = catchAsync(async (req, res, next)=>{
    ...
    // 1.pass email and password
    const {email, password} = req.body;


// 2.check email or password exist.
    if(!email||!password){
        return next(new AppError("Please provide email and password!", 400));
    }
  
    // 3.check if user is exists and  password is correct
...
exports.protect = catchAsync(async(req, res, next)=>{
    ...
    //3 check if user still exist
     ...
     if(!currentUser){
         return next(new AppError('the token belong to the user no longer exist.', 401));
     }
   //4 check user changed the password, after the token issued
     if(currentUser.changedPasswordAfter(decode.iat)){
           return next(new AppError("User Recently changed the password, Please login again!", 401));
     }


    //5 Grant acces to  proctected route
...
exports.forgotPassword = catchAsync (async (req, res, next) => {
    ...
    // 3) Send to it user's Email
    
    // const message = `Forgot your password? Submiy a request with your new password and pasword to:\n 
    // ${resetURL}.\n
  ...
  try{
    ...
    await new Email(user, resetURL).sendPasswordReset();
      ...
      res.status(200).json({
          status:'success',
          message: 'Reset password link send successfully check you email'
      });
    }catch(err){
